...
$alert = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_tuition_fee'])) {
    ...
    if ($admission_no && $class) {
        $sql = "INSERT INTO tuition_fees (student_id, class, amount, description, date_paid) VALUES ('$admission_no', '$class', '$amount', '$description', '$date_paid')";
...
<head>
    ...
    function updateClassAndInfo() {
        ...
        if (selected && selected.value !== "") {
            ...
            iframe.src = "tuition_fees.php?student_info=" + encodeURIComponent(selected.value);
    ...
    function openPaid() {
        window.location.href = "paid_tution_fees.php";
    }
    function openUnpaid() {
        window.location.href = "unpaid_tution_fees.php";
    }
    </script>
</head>
<body>
...
<header class="header">
    ...
    <span class="dashboard-title" style="margin: 0 auto; display: flex; align-items: center; justify-content: center; flex: 1; text-align: center;">
        ...
        <span style="margin-left: 8px;">Tuition Fees</span>
    </span>
    <div class="logout">
        <a href="logout.php" class="btn btn-danger">Logout</a>
    </div>
...
<div class="main-flex-container">
    ...
    <div class="main-container">
        <h2><i class="fa fa-graduation-cap"></i> Tuition Fees Payment</h2>
        ...
        <form method="post" action="tuition_fees.php">
            ...
            <div class="form-group">
                ...
                <input type="text" name="description" id="description" class="form-control" required placeholder="E.g. Tuition Fees 2024">
            </div>
            <div class="form-group">
                <label for="date_paid">Date Paid:</label>
                <input type="text" name="date_paid" id="date_paid" class="form-control" value="<?php echo date('Y-m-d H:i:s'); ?>" readonly>
            </div>
            <button type="submit" name="add_tuition_fee" class="btn btn-success">
                <i class="fa fa-plus"></i> Add Tuition Fee
            </button>
        </form>
        <?php if ($alert === "success"): ?>
            <div class="alert alert-success">Tuition fee payment added successfully!</div>
        <?php elseif ($alert === "error"): ?>
            <div class="alert alert-danger">
                Failed to add tuition fee payment.
