...
$alert = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_exam_fee'])) {
    ...
    if ($admission_no && $class) {
        $sql = "INSERT INTO examination_fees (student_id, class, amount, description, date_paid) VALUES ('$admission_no', '$class', '$amount', '$description', '$date_paid')";
...
<html>
<head>
    <title>Examination Fees Payment</title>
    ...
    <style>
        ...
        .btn-success { background: #f59e42; border: none; color: #fff; }
    ...
    function updateClassAndInfo() {
        ...
        if (selected && selected.value !== "") {
            ...
            iframe.src = "examination_fees.php?student_info=" + encodeURIComponent(selected.value);
    ...
    function openPaid() {
        window.location.href = "examination_fees_paid.php";
    }
    function openUnpaid() {
        window.location.href = "examination_fees_unpaid.php";
    }
    </script>
</head>
<body>
...
<header class="header">
    <button class="sidebar-toggle" id="sidebarToggle" aria-label="Toggle admin panel">
        ...
        <span style="margin-left: 8px;">Examination Fees</span>
    </span>
    <div class="logout">
...
<div class="main-flex-container">
    ...
    <div class="main-container">
        <h2><i class="fa fa-file-alt"></i> Examination Fees Payment</h2>
        ...
        <form method="post" action="examination_fees.php">
            ...
            <div class="form-group">
                ...
                <input type="text" name="description" id="description" class="form-control" required placeholder="E.g. Exam Fees 2024">
            </div>
            <div class="form-group">
                <label for="date_paid">Date Paid:</label>
                <input type="text" name="date_paid" id="date_paid" class="form-control" value="<?php echo date('Y-m-d H:i:s'); ?>" readonly>
            </div>
            <button type="submit" name="add_exam_fee" class="btn btn-success">
                <i class="fa fa-plus"></i> Add Examination Fee
            </button>
        </form>
        <?php if ($alert === "success"): ?>
            <div class="alert alert-success">Examination fee payment added successfully!</div>
        <?php elseif ($alert === "error"): ?>
            <div class="alert alert-danger">
                Failed to add examination fee payment.
