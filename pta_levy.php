...
$alert = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_levy'])) {
    ...
    if ($admission_no && $class) {
        $sql = "INSERT INTO pta_levy (student_id, class, amount, description, date_paid) VALUES ('$admission_no', '$class', '$amount', '$description', '$date_paid')";
...
<head>
    ...
    function updateClassAndInfo() {
        ...
        if (selected && selected.value !== "") {
            ...
            iframe.src = "pta_levy.php?student_info=" + encodeURIComponent(selected.value);
    ...
    function openPaid() {
        window.location.href = "pta_levy_paid.php";
    }
    function openUnpaid() {
        window.location.href = "pta_levy_unpaid.php";
    }
    </script>
</head>
<body>
...
<header class="header">
    <button class="sidebar-toggle" id="sidebarToggle" aria-label="Toggle admin panel">
        ...
        <i class="fa fa-btn-group"></i>
        <span style="margin-left: 8px;">PTA LEVY</span>
    </span>
    <div class="logout">
...
<div class="main-flex-container">
    ...
    <div class="main-container">
        <h2><i class="fa fa-users"></i> PTA Levy Payment</h2>
        ...
        <form method="post" action="pta_levy.php">
            ...
            <div class="form-group">
                ...
                <input type="text" name="description" id="description" class="form-control" required placeholder="E.g. PTA Levy 2024">
            </div>
            <div class="form-group">
                <label for="date_paid">Date Paid:</label>
                <input type="text" name="date_paid" id="date_paid" class="form-control" value="<?php echo date('Y-m-d H:i:s'); ?>" readonly>
            </div>
            <button type="submit" name="add_levy" class="btn btn-success">
                <i class="fa fa-plus"></i> Add PTA Levy
            </button>
        </form>
        <?php if ($alert === "success"): ?>
            <div class="alert alert-success">PTA levy payment added successfully!</div>
        <?php elseif ($alert === "error"): ?>
            <div class="alert alert-danger">
                Failed to add PTA levy payment.
